        ...
        className="space-y-8 p-10">
            {EXPERIENCES.map((experinece, index)=> (
                    ...
                    key={index} className="rounded-xl border border-stone-50/30 bg-white/10 p-4">
                        <h3 className="text-2xl font-semibold">{experinece.title}</h3>
                        <p className="text-xl">{experinece.company}</p>
                        <p className="tet-sm text-stone-300">{experinece.duration}</p>
                        <p className="mt-2 text-base">{experinece.description}</p>
                    </motion.div>
                ))}
        </motion.div>
